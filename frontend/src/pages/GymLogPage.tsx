...
/** =============== Date helpers =============== */
const isSameWeek = (a: Date, b: Date) => startOfWeek(a).getTime() === startOfWeek(b).getTime();
...
export function GymLogPage() {
    ...
    useEffect(() => {
        if (!isSameWeek(selectedDate, weekAnchor)) {
    ...
    const shiftWeek = (days: number) => {
        setWeekAnchor((a) => {
            ...
            // Giữ nguyên offset “thứ” của selectedDate trong tuần hiện tại
            const curStart = startOfWeek(selectedDate);
            const offsetDays = Math.max(
                0,
                Math.min(6, Math.round((parseISO(fmtISO(selectedDate)).getTime() - curStart.getTime()) / (24 * 3600 * 1000)))
            );

            const newSelected = addDays(newStart, offsetDays);
...
function WeekScroller({
    ...
    today: Date;
    monthMap: LogsDB;
    onShift: (days: number) => void; // NEW
}) {
    const start = startOfWeek(anchorDate);
    const days = Array.from({ length: 7 }, (_, i) => addDays(start, i));

    // range hiển thị ví dụ "Sep 21–27, 2025"
