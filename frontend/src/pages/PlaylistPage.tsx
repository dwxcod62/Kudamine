import { Heart, Pause, Play, Plus, Search, SkipBack, SkipForward, Trash2, Volume2, VolumeX } from "lucide-react";
...
import defaultCover from "../assets/playlist-cover.jpg";
...
export default function PlaylistPage() {
    ...
    // const USER_ID = "66b4e806-6799-48f5-ac9d-fe5cad1df298";
    ...
    const saveCover = async (input: File | string | null) => {
        if (!playlist || !input) return;

        if (typeof input === "string") {
            setPlCover(input);
            try {
                const res = await fetch(`http://localhost:3000/playlists/${playlist.id}`, {
                    method: "PATCH",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({ coverUrl: input }),
                });
            ...
            setPlCover(URL.createObjectURL(input));

            try {
                const form = new FormData();
                form.append("cover", input);
                const res = await fetch(`http://localhost:3000/playlists/${playlist.id}`, {
                    method: "PATCH",
                    body: form,
                });
